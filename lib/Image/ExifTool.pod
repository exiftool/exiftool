...
External character set used when specifying file names.  When set in
Windows, this triggers use of Windows Unicode file library routines. 
Default is undef.
...
Jpeg2000, LNK, Leaf, Lytro, M2TS, MIE, MIFF, MNG, MPC, MPEG, MPF, MXF, MOI,
...
The encoding of file and directory names is a special case.  By default,
file names passed to ExifTool are used directly in calls to the system i/o
routines (which expect UTF-8 strings on Mac/Linux, but default to the system
code page on Windows), however this may be changed by setting the
L</CharsetFileName> option.  In Windows, setting L<CharsetFileName> also has
the effect of activating Unicode filename support, and causes the special
Windows wide-character i/o routines to be used if available.
...
L</CharsetEXIF>, L</CharsetFileName>, L</CharsetID3>, L</CharsetIPTC>,
L</CharsetPhotoshop> and L</CharsetQuickTime> options.
