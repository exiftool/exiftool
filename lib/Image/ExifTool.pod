...
Windows, this triggers use of Windows wide-character i/o library routines
...
The encoding of file names is a special case.  By default, file names passed
to ExifTool are used directly in calls to the system i/o routines (which
expect UTF-8 strings on Mac/Linux, but default to the system code page on
Windows), however this may be changed by setting the L</CharsetFileName>
option.  In Windows, setting L</CharsetFileName> also has the effect of
activating Unicode filename support via the special Windows wide-character
i/o routines if Win32API::File is available.
