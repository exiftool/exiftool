...
input into tag name expresssions and ValueConv logic. I<PARAM> is the
user-defined parameter name (not case sensitive).

  PARAM         - Get parameter
  PARAM=        - Clear parameter
  PARAM=VALUE   - Set parameter

User-defined parameters are accessed in tag name expressions by prefixing
the parameter name with a dollar sign, just like normal tags.
