...
set to 3, the file is not actually parsed to extract metadata -- pseudo
System tags are generated and the file header is read only to provide an
educated guess at FileType.  When set to 4, the file is not read at all, and
FileType is determined based on the file's extension.  Default is undef.
