...
=item QuickTimePreservePadding

Flag set to preserve the padding of tags within the QuickTime Container.
The default behaviour is to size each tag with as much space as needed
exactly; however some files (like Canon CR3) include padding into each of
the tags. Setting this flag to 1 will maintain the space allocated to each
tag by appending NUL bytes at the end if a rewritten tag is shorter than its
original. If a rewritten tag is longer than the original, the new longer tag
